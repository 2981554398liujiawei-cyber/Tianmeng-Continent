...
const fight = async () => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (cb.includes('返回冒险')) { console.log(`R${i}: 结束 ${cb.includes('战斗失败') ? 'LOSE' : cb.includes('战斗胜利') ? 'WIN' : '?'} 玩家HP=${hpM?.[1]}/${hpM?.[2]} MP=${mpM?.[1]} 敌HP=${ehpM?.[1]}/${ehpM?.[2]}`); break }
    if (!cb.includes('普通攻击')) { console.log(`R${i}: 无普攻 玩家HP=${hpM?.[1]}/${hpM?.[2]} MP=${mpM?.[1]} 敌HP=${ehpM?.[1]}/${ehpM?.[2]}`); break }
    ...
    if (skillOpen) {
      ...
      if (skillUsed) { console.log(`R${i}: 技能 玩家HP=${hpM?.[1]}/${hpM?.[2]} MP=${mpM?.[1]} 敌HP=${ehpM?.[1]}/${ehpM?.[2]}`); await sleep(450); continue }
      console.log(`R${i}: 技能disabled 玩家HP=${hpM?.[1]}/${hpM?.[2]} MP=${mpM?.[1]} 敌HP=${ehpM?.[1]}/${ehpM?.[2]}`)
    ...
    if (hpM && Number(hpM[1]) / Number(hpM[2]) < 0.7) {
      ...
      if (used) { console.log(`R${i}: 用药 玩家HP=${hpM?.[1]}/${hpM?.[2]} 敌HP=${ehpM?.[1]}/${ehpM?.[2]}`); await sleep(450); continue }
      await page.evaluate(() => [...document.querySelectorAll('button')].find((b) => b.textContent.includes('物品'))?.click())
      await sleep(150)
    }
    await page.evaluate(() => [...document.querySelectorAll('button')].find((b) => b.textContent.includes('普通攻击'))?.click())
    console.log(`R${i}: 普攻 玩家HP=${hpM?.[1]}/${hpM?.[2]} MP=${mpM?.[1]} 敌HP=${ehpM?.[1]}/${ehpM?.[2]}`)
    await sleep(450)
  }
...
await fight(); await clickBtn('返回冒险'); await clickBtn('青石村'); await sleep(300); await submitAndClose()

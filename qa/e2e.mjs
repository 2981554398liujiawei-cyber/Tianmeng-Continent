// 一次性 E2E QA 脚本：验证 TM-P0-001 验收标准 B/D/E（主菜单 → 新游戏 → 状态修改 → 存读档）
...
try {
  ...
  check('无存档时「继续游戏」禁用', continueDisabled === true)
  ...
  await clickByText('返回主菜单')
  const continueDisabled2 = await page.evaluate(() => {
    const btn = [...document.querySelectorAll('button')].find((b) => b.textContent.includes('继续游戏'))
    return btn ? btn.disabled : null
  })
  check('删除存档后「继续游戏」重新禁用', continueDisabled2 === true)

  // 异常存档回退（验收 F）：注入非法 localStorage 后刷新不得白屏
  await page.evaluate(() => {
  ...
  await sleep(500)
  body = await bodyText()
  check('损坏存档刷新不白屏（主菜单可见）', body.includes('天梦大陆'))
} catch (err) {

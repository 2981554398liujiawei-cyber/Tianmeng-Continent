...
const rest = async () => { const c = await page.evaluate(() => { const b = [...document.querySelectorAll('button')].find((el) => el.textContent.trim() === '休整'); if (b && !b.disabled) { b.click(); return true } return false }); if (c) await sleep(500) }
...
await fightFast(); await clickBtn('返回冒险'); await clickBtn('青石村'); await sleep(300); await submitAndClose()
await acceptQuest('矿洞清理'); await rest(); await clickBtn('废弃矿洞'); await clickBtn('迎战')
await page.waitForSelector('[data-testid="combat-summary-feed"]', { timeout: 8000 }); await sleep(300)
await fightFast(); await clickBtn('返回冒险'); await clickBtn('青石村'); await sleep(300); await submitAndClose()
await acceptQuest('草原狼影'); await rest(); await clickBtn('村外草原'); await sleep(300)
// 选择魔化狼的迎战（若多个迎战按钮，选含「魔化狼」行内的）
const wolfOk = await page.evaluate(() => {
  const btns = [...document.querySelectorAll('button')].filter((b) => b.textContent.includes('迎战'))
  const row = btns.find((b) => b.parentElement?.textContent?.includes('魔化狼'))
  if (row && !row.disabled) { row.click(); return true }
  return false
})
console.log('魔化狼迎战:', wolfOk)
if (!wolfOk) await clickBtn('迎战')
await page.waitForSelector('[data-testid="combat-summary-feed"]', { timeout: 8000 }); await sleep(300)
await fightFast(); await clickBtn('返回冒险'); await clickBtn('青石村'); await sleep(300); await submitAndClose()
await rest(); await clickBtn('村外草原'); await sleep(300); await clickBtn('兔王巢穴'); await sleep(300)
await clickBtn('迎战')
await page.waitForSelector('[data-testid="combat-summary-feed"]', { timeout: 8000 }); await sleep(500)
// ===== 嘟嘟兔精细观测 =====
await page.evaluate(() => {
  window.__origRandom = Math.random.bind(Math)
  const calls = []
  window.__calls = calls
  Math.random = () => { calls.push(new Error().stack?.split('\n')[2]?.trim() || '?'); return 0.99 }
})
...
await state('S0')
await page.evaluate(() => { const x = [...document.querySelectorAll('button')].find((el) => el.textContent.includes('骑士重击')); if (x && !x.disabled) x.click() })
await sleep(100); await state('S1a')
await sleep(200); await state('S1b')
await sleep(200); await state('S1c')
const calls = await page.evaluate(() => window.__calls.slice(0, 12))
console.log('RANDOM_CALLS:', JSON.stringify(calls))
await page.evaluate(() => { Math.random = window.__origRandom })

...
> 当前阶段：Phase 2 — 角色自由创建、响应式界面、战斗规则 V3 与五槽位存档（任务卡 TM-P2-001 / TM-P2-002）。
> Phase 2 已上线正式可玩内容：角色由玩家自由创建（姓名 / 职业 / 属性分配 / 职业推荐配点）、
> 桌面三栏 / 平板两栏 / 手机单栏响应式界面、战斗规则 V3（攻击力/护甲/敏捷 + 敏捷命中 + 护甲减伤 + 先手）、
> 五槽位存档（自动迁移旧单档、导出/导入）、以及新主线《北门失联》（天龙城北门 → 痕迹调查 → 黑鬃魔狼 → 断裂铜牌 → 回武馆复命）。

## 阶段状态

> **PHASE 1 — SEALED ✅**（用户已确认通过 GPT 最终源码验收；当前 HEAD 为 `e0de5b0`）。
...
> 当前自测基线：单测 917、Dev E2E 953、phase1-playthrough 195、phase2-e2e 58、responsive-e2e 20、prod smoke 6、build PASS、GitHub Actions CI success。

## 战斗系统（V3）

- 玩家属性：攻击力 / 护甲等级 / 敏捷（角色面板与战斗面板展示）
- 命中：D20；天然 1 → 大失败 0 伤害；天然 20 → 200% 暴击；否则 (攻击者敏捷 + roll)/2 ≥ 对方敏捷 → 命中，否则擦伤（50% 向上取整）
- 护甲：非 MISS 攻击最终伤害 = max(1, ceil(原始伤害 × roll/(armor+roll)))；护甲越高减伤越多，但至少 1 点
- 先手：进入战斗双方各掷 D20+AGI，高者先；平局 AGI 高者先；仍相同玩家先；敌人先手则敌人先攻击一次
- 技能（普通/法术/骑士重击/迅捷突袭/压制猛击）统一走 V3 命中与护甲（技能特色=原始伤害公式，后续卡统一重构）

## 存档系统（V2 五槽位）

- 五个槽位（Slot 1–5），逐槽独立存储；一个槽损坏不影响其他槽读取
- 游戏内「保存游戏」→ 五槽位选择（已有存档需确认覆盖）；主菜单「继续游戏」读最近存档、「读取存档」进入五槽位页面（可删除槽位）
- 旧 V1 单档自动迁移至 Slot 1（迁移成功前不删除旧 key）
- 「导出存档 / 导入存档」：五槽位整体 JSON 备份，完整校验版本与结构，非法导入不覆盖现有存档

## 幸运（Luck）

- 角色面板与战斗面板正确展示幸运；`src/game/rules/luck.ts` 提供稳定规则入口（getLuckModifier / 应用说明）
- 幸运后续用途（TM-P2-003 起实现，本阶段不塞假随机数）：战利品质量、战利品数量、宝箱、幸运场景检定、机缘型社交
...
- R1（解锁 UI 守卫对齐 + 截图清理）：GamePage 新增 towerUnlockPending（unlock flag 严格 undefined/false 视为「待解锁」）并替换解锁入口条件为 tianlong_city + towerQuestInProgress（复用 in_progress/stage 0 常量，不重复写表达式）+ wangcaiBriefed + towerUnlockPending——stage!=0 或 unlock flag 异常非 boolean（"yes"/1/0.5）或已 true 一律不显示「动身调查黑石塔」，消除 UI 允许但 Store 拒绝的死按钮（P1-023-R1 同类型）；qa/dev-state.png 恢复为 P1-024 基线版本（git diff a66c7d7..HEAD 无该文件 diff）；Store/App/战斗逻辑零修改；E2E 新增 9 项注入验证：unlock=false+stage0 → 入口存在 / stage=1 → 无按钮 / unlock="yes" → 无按钮 / unlock=1 → 无按钮 / unlock=0.5 → 无按钮 / 恢复合法档 → 黑石塔一层清场状态零回归+无动身调查按钮

TM-P1-026（黑石塔一层：骷髅队长 Boss 战与项链线索推进——一层 Boss 一次性清场；P1-027 才建黑石塔二层）：
- enemies.ts 新增 skeleton_captain：Lv.4 骷髅队长（tags ['undead','boss']、maxHp 22、defense 13、attackBonus 4、damage 4）——黑石塔一层骷髅士兵头领；继续现有普通确定性 D20 战斗；无骨刺/伤害反弹/重斩/击晕/Boss 技能系统/亡灵抗性/特殊 AI/掉落
- locations.ts black_stone_tower_floor1.enemyIds ['skeleton_soldier']→['skeleton_soldier','skeleton_captain']（同一场景节点，不建新大厅 Location）；connections 仍精确 ['tianlong_city']；未创建 black_stone_tower_floor2/三层/骷髅女妖
- 骷髅队长可见前置（GamePage visibleEnemies 窄条件）：当前位置 black_stone_tower_floor1 + 第五主线 in_progress/stage 0 + wangcai_briefed===true + black_stone_tower_unlocked===true + floor1_soldier_defeated===true + floor1_captain_defeated undefined/false（异常非 boolean 不显示）
- captain flag 安全：quest.flags.floor1_captain_defeated 只允许 undefined/false/true；"yes"/1/0.5 → Boss 不显示 + App 不允许进入战斗 + resolveCombatVictory 不允许提交胜利（不自动修复）
- App.handleEngage('skeleton_captain') 窄守卫完整重查（位置+任务 in_progress/stage 0+briefed===true+unlocked===true+soldier_defeated===true+captain undefined/false；true/非 boolean 拒绝）；未建 bossGate/questEnemyGate/EncounterGuard
- resolveCombatVictory('skeleton_captain') 完整前置守卫在 ok=true 之前（士兵未击败/quest 不存在/非 in_progress/stage!=0/briefed 非 true/unlocked 非 true/captain true 或非 boolean → false 且 GameState 完全不变）；首次合法胜利只写 quest.flags.floor1_captain_defeated=true（status 保持 in_progress/stage 0/wangcai_briefed 保持 true/soldier_defeated 保持 true）；禁止 completable/completed/stage+1
- Boss 无即时奖励：无金币/经验/等级/属性/装备/道具/声望/关系值/治疗；未创建骷髅大剑/黑石/骨头/骷髅套装物品
- Boss 清场：floor1_captain_defeated=true 后骷髅队长消失、无迎战按钮；士兵也已清场 → 整个「附近威胁」section 不显示；往返与 Save/Continue 不复活
- Boss 后固定剧情（GamePage 大厅深处块双态）：战前（soldierDefeated && !captainDefeated）「大厅中的骷髅士兵已经被击败。/更深处传来沉重的骨骼碰撞声，一名身材高大的骷髅队长守在前方。」（删除 P1-025 的「骷髅队长：【待开放】」）；战后（captainDefeated）「骷髅队长已经倒下。/你检查了骷髅队长与周围，没有发现夔峒项链。/通往黑石塔更深处的道路仍需继续调查。/黑石塔二层：【待开放】」（无按钮；未出现骷髅女妖/三层/项链在某人手里）
- 第五主线日志五态：战前「当前目标：击败骷髅队长。」；战后「黑石塔一层：骷髅队长已击败，未发现夔峒项链。/当前目标：继续深入黑石塔。/黑石塔二层：【待开放】」（任务仍 in_progress，无可完成/提交/奖励）
- 黄金兔子长期线冻结：in_progress/stage 0/四 flag true/rabbit_path ×1；未建二层/三层/骷髅女妖/夔峒项链 ItemDefinition
...
- content 测试：ENEMIES toHaveLength(6) + 黑石塔一层 enemyIds 精确 ['skeleton_soldier','skeleton_captain'] + 骷髅队长完整锁定（level 4/tags undead+boss/maxHp 22/defense 13/attackBonus 4/damage 4）+ 无 skeleton_witch/black_stone_tower_floor2 + LOCATIONS/NPCS/QUESTS/ITEMS 数量不变
- Store 单测 25 项 A-Q：A wrong location false/B quest 不存在 false/C available/completable/completed false（it.each）/D stage!=0 false/E briefed 非 true（undefined/false/"yes"/1）false/F unlocked 非 true false/G soldier_defeated 非 true（undefined/false）false/H captain undefined 首次胜利 true/I captain false 首次胜利 true/J 成功 floor1_captain_defeated=true/K captain=true 重复 false 同一引用/L captain 非 boolean（"yes"/1/0.5）false 同一引用原值保留/M 成功后 status in_progress/stage 0/briefed true/soldier true/N 无金币/物品/装备/等级奖励（无大剑/黑石/骨头/骷髅物品）/O 除本 Quest flag 外 GameState 全不变/P 黄金兔子 QuestState 整体深比较不变/Q 不自动保存
- 34 项 E2E（直接继续 P1-025 Save/Continue 后的黑石塔一层清场档：A Continue 后任务进行中+已击败骷髅士兵/B Boss 正式出现（无【待开放】）+附近威胁仅骷髅队长 Lv.4+无威胁卡片骷髅士兵+当前目标：击败骷髅队长/C 记录战前 Lv/maxHP/maxMP/gold/地图数/背包/D Math.random 隔离 0.99 确定性 Boss 战胜利/E 骷髅队长已经倒下+未发现夔峒项链+通往更深处的道路仍需继续调查+黑石塔二层：【待开放】+无附近威胁/迎战/F 任务进行中+骷髅队长已击败未发现项链+当前目标：继续深入黑石塔+无可完成/G 战前后 Lv/maxHP/maxMP/gold/地图数/背包精确全不变（HP/MP 允许战斗变化）/H 往返：天龙城可前往精确 [武馆,黑石塔一层]+骷髅队长/士兵不复活+Boss 后剧情仍在+一层移动按钮精确 [天龙城] 无二层/I Save/Continue 后当前位置 floor1+两 defeated flag+任务进行中+当前目标继续深入+黑石塔二层：【待开放】+无附近威胁+黄金主线进行中+兔子的路径 ×1；另按任务卡「历史 E2E 到期调整」窄改 P1-025-E/F/H/I/R1 断言：骷髅队长：【待开放】→ 删除、日志「继续深入，处理骷髅队长」→「击败骷髅队长」、附近威胁仅骷髅队长、「无骷髅士兵」精确为威胁卡片文本 骷髅士兵 · Lv.3（任务日志「已击败骷髅士兵。」仍含该词不误伤））

TM-P1-027（黑石塔二层：武馆休整、僵尸与黑法师——二层入口固定顺序战斗；二层深处与三层 P1-028 起才开放）：
- 天龙城武馆免费休整 restAtTianlongMartialHall()：仅 tianlong_martial_hall 可用——hp=maxHp、mp=maxMp；HP=0 时允许、任一资源未满即可成功；HP/MP 全满 → false 且完整 GameState 不变；成功只改 character.hp/mp（金币/XP/等级/物品/装备/Quest/flags/关系/npcState 全不变）；不自动保存；未建 RestSystem（GamePage 武馆「武馆休整」块，非武馆地点不显示）
- locations.ts 新增 black_stone_tower_floor2：id/name 黑石塔二层/description「曲折的黑石通道向深处延伸，腐败气息与幽暗魔力混在潮冷的空气中。」/requiredFlag black_stone_tower_floor2_unlocked/connections ['black_stone_tower_floor1']/enemyIds ['tower_zombie','black_mage']；black_stone_tower_floor1.connections → ['tianlong_city','black_stone_tower_floor2']（天龙城 ↔ 一层 ↔ 二层；二层未解锁时移动按钮 disabled 复用 requiredFlag）；未建 black_stone_tower_floor3
- Store unlockBlackStoneTowerFloor2()：位置 black_stone_tower_floor1 + 第五主线 in_progress/stage 0 + wangcai_briefed===true + black_stone_tower_unlocked===true + floor1_soldier_defeated===true + floor1_captain_defeated===true + target flag black_stone_tower_floor2_unlocked undefined/false → 成功原子写 world.flags.black_stone_tower_floor2_unlocked=true；target 已 true 或非 boolean（"yes"/1/0.5）整次拒绝且完全不变（不自动修复）；player/inventory/equipment/quests/其他 world.flags/npcStates/completedEvents 全不变；不自动保存
- 黑石塔一层 UI：满足全部前置（士兵+队长均已击败）后显示「继续深入」块，按钮只调用 unlockBlackStoneTowerFloor2()（UI 不直接写 flag）；target flag 异常非 boolean 或已 true 时不显示（避免死按钮，P1-025-R1 同模式）
- enemies.ts 新增 tower_zombie（Lv.4 僵尸、undead、maxHp 18、defense 12、attackBonus 4、damage 4——入口固定顺序第一场；无中毒/吸血/持续伤害/特殊恢复）+ black_mage（Lv.4 黑法师、undead、maxHp 14、defense 11、attackBonus 5、damage 4——第二场；无盲目/暗属性/黑色火球/暴躁/特殊法术 AI）；继续现有普通确定性 D20 战斗
- 二层固定顺序（GamePage visibleEnemies 严格 boolean）：进入二层只显示 tower_zombie；floor2_zombie_defeated===true 后僵尸消失、black_mage 出现；floor2_black_mage_defeated===true 后两者均不显示（所有判断 ===true，malformed truthy 不绕过）
- App.handleEngage 三层守卫：'tower_zombie'（位置 floor2+任务 in_progress/stage 0+briefed===true+unlocked===true+floor2_unlocked===true+soldier===true+captain===true+zombie undefined/false）；'black_mage' 额外要求 floor2_zombie_defeated===true（僵尸未击败不得提前 engage 黑法师）+ mage undefined/false；不满足 return（不进入战斗）
- resolveCombatVictory('tower_zombie')：完整前置守卫在 ok=true 之前（位置/quest in_progress/stage 0/briefed===true/unlocked===true/floor2_unlocked===true/soldier===true/captain===true/zombie undefined|false；已 true 或非 boolean 拒绝）；合法首次胜利只写 quest.flags.floor2_zombie_defeated=true（status/stage 不变；无 XP/金币/等级/物品/装备/关系/npcState/world flag/奖励）
- resolveCombatVictory('black_mage')：额外要求 floor2_zombie_defeated===true；合法首次胜利只写 quest.flags.floor2_black_mage_defeated=true（status/stage 不变；无奖励/治疗）
- 二层清场剧情（两 flag 均 true）：「二层前段的僵尸与黑法师已经被清理。/曲折的通道继续向深处延伸，前方小厅中出现了更强的骷髅战士，挡住继续深入的道路。/黑石塔二层深处：【待开放】」（骷髅战士只作为剧情文本；未建 skeleton_warrior EnemyDefinition；无按钮）
- 第五主线日志六态：最终态「黑石塔一层：已击败骷髅士兵。/黑石塔一层：骷髅队长已击败，未发现夔峒项链。/黑石塔二层：入口区域已清理。/当前目标：继续向黑石塔二层深处推进。/黑石塔二层深处：【待开放】」（status 仍 in_progress/stage 0，不可提交/领奖；禁止返回王财交任务/获得奖励/获得夔峒项链/进入三层）
- 黄金兔子长期线冻结：in_progress/stage 0/四 flag true/rabbit_path ×1；未创建三层/骷髅战士/骷髅女妖/夔峒项链 ItemDefinition；无新 NPC/Quest；无毒/吸血/盲目/暗属性/黑色火球/暴躁/掉落系统/腐肉/黑法师结晶/解毒药；无 RestSystem/DungeonEngine/EncounterManager/LootSystem
- schema 不变、SAVE_VERSION=1；quests.ts/npcs.ts/items.ts/professions.ts/CombatPage.tsx/types/storage/rules 零修改；git diff 仅 enemies.ts + locations.ts + content.test.ts + gameStore.ts + gameStore.test.ts + GamePage.tsx + App.tsx + qa/e2e.mjs + README（不含 qa/*.png）
- content 测试：LOCATIONS toHaveLength(8)（+黑石塔二层）+ 一层 connections 精确 ['tianlong_city','black_stone_tower_floor2'] + 二层注册表完整锁定（id/name/description/requiredFlag/connections/enemyIds）+ ENEMIES toHaveLength(8) + 僵尸/黑法师完整锁定（level 4/undead/18-12-4-4 与 14-11-5-4）+ 无 skeleton_warrior/black_stone_tower_floor3/骷髅女妖 + NPC/QUEST/ITEM 数量不变
...
TM-P1-029（黑石塔三层：骷髅女妖与夔峒项链——开放三层 → 击败骷髅女妖 → 找到夔峒项链；交还王财留待下一卡，本卡不完成第五主线）：
- locations.ts 新增 black_stone_tower_floor3：id/name 黑石塔三层/description「越过石阶后，塔内变得更加阴冷。残破石柱围绕着中央厅堂，低沉的哭嚎声从黑暗中传来。」/requiredFlag black_stone_tower_floor3_unlocked/connections ['black_stone_tower_floor2']/enemyIds ['skeleton_witch']；black_stone_tower_floor2.connections → ['black_stone_tower_floor1','black_stone_tower_floor3']（二层未解锁三层时移动按钮 disabled）；LOCATIONS 9 个；未建四层
- Store unlockBlackStoneTowerFloor3()：仅在二层 + 第五主线 in_progress/stage 0 + briefed===true + black_stone_tower_unlocked===true + floor2_unlocked===true + floor1_soldier/captain + floor2_zombie/black_mage/skeleton_warrior 全部 ===true + 目标 flag black_stone_tower_floor3_unlocked undefined/false → 成功原子写 world.flags.black_stone_tower_floor3_unlocked=true；目标已 true 或非 boolean（"yes"/1/0.5）整次拒绝且完全不变；不建 DungeonEngine/楼层系统；player/inventory/equipment/quests/其他 world.flags/npcStates/completedEvents 全不变；不自动保存
- P1-028 战后「黑石塔三层：【待开放】」改为正式「继续向上」入口；UI 只调用 Store action（不直接写 flag）；target 异常非 boolean/已 true 不显示入口（避免死按钮）
- enemies.ts 新增 skeleton_witch：Lv.5、tags ['undead']、maxHp 18、defense 12、attackBonus 5、damage 5、name 骷髅女妖；继续现有普通确定性 D20 战斗；不因「女妖」增加法术系统——无诅咒/恐惧/吸血/灵魂攻击/暗属性/召唤/特殊 AI
- skeleton_witch 出现条件（GamePage visibleEnemies 严格 ===true）：location=black_stone_tower_floor3 + 全部前序严格 true（含二层三敌均击败 + floor3_unlocked===true）+ floor3_skeleton_witch_defeated undefined/false；"yes"/1/0.5 malformed 拒绝
- UI guard + App.handleEngage guard + Store resolveCombatVictory guard 三层一致（skeleton_witch 战斗入口与胜利均需全前置；malformed witch flag 拒绝且完整 GameState 不变）
- items.ts 新增 kuidong_necklace：id/name 夔峒项链/type quest/description「王财在黑石塔附近遭遇魔物袭击时遗失的项链，是他妻子留下的重要物件。」/value 0；复用现有 ItemDefinition quest 类型，不新建任务物品系统；ITEMS 6 个
- resolveCombatVictory('skeleton_witch')：合法首次胜利原子完成——只写 quest.flags.floor3_skeleton_witch_defeated=true + 获得夔峒项链 ×1（inventory 只允许一条 entry 不重复堆叠）；任务保持 status=in_progress/stage=0 不设 completable（正式 giver 是马科，直接走 generic completable 会把玩家引回武馆破坏剧情）；无金币/XP/等级/装备/治疗/关系/npcState/其他 world flag/战后奖励；重复拒绝；不自动保存
- 女妖击败后剧情（三层厅堂块）：「骷髅女妖倒在破碎的石柱之间。/你在厅堂深处搜索时，发现了一条被灰尘覆盖的项链。/这正是王财所说的夔峒项链。/夔峒项链 ×1 已获得。/当前目标：返回天龙城，将夔峒项链交还王财。」
- 任务日志最终态（八态）：「黑石塔一层：已击败骷髅士兵。/黑石塔一层：骷髅队长已击败，未发现夔峒项链。/黑石塔二层：入口区域已清理。/黑石塔二层深处：骷髅战士已击败，仍未发现夔峒项链。/黑石塔三层：骷髅女妖已击败。/黑石塔三层：已找到夔峒项链。/当前目标：返回天龙城，将夔峒项链交还王财。」；quest_wangcai_trouble 仍不可提交
- 黄金兔子长期线继续完全冻结：in_progress/stage 0/四 flag true/rabbit_path ×1；未建 黑石塔四层/BossSystem/DungeonEngine/LootSystem/女妖技能系统/新 NPC/新 Quest/新装备/新药水
- schema 不变、SAVE_VERSION=1；quests.ts/npcs.ts/professions.ts/CombatPage.tsx/types/storage/rules 零修改；git diff 仅 enemies.ts + locations.ts + items.ts + content.test.ts + gameStore.ts + gameStore.test.ts + GamePage.tsx + App.tsx + qa/e2e.mjs + README（不含 qa/*.png）
- content 测试：LOCATIONS toHaveLength(9)（+黑石塔三层完整锁定）+ 二层 connections ['black_stone_tower_floor1','black_stone_tower_floor3'] + ENEMIES toHaveLength(10) + skeleton_witch 完整锁定（level 5/undead/18-12-5-5）+ ITEMS toHaveLength(6) + kuidong_necklace 完整锁定（type quest/value 0）+ NPC/QUEST 数量不变
- Store 单测新增 U3 组（unlock floor3：U1 合法只写 target flag/U2 it.each 16 项拒绝 false 且完整不变/U3 warrior 未击败拒绝/U4 不 autosave）+ V 组（witch：V1 前序未全部击败（非三层/三层未解锁）拒绝/V2 合法胜利 flag+项链 ×1/V3 任务保持 in_progress/stage 0 不设 completable/V4 无金币/XP/等级/装备奖励/V5 重复胜利 false 项链不堆叠/V6 it.each 12 项拒绝（错误 location/任务不存在/available/stage 1/briefed false/三层未解锁/soldier/captain/warrior/witch true/"yes"/1/0.5）/V7 黄金深比较不变/V8 不 autosave）
- E2E 新增 P1-029 段（继续 P1-028-H 二层 warrior 已击败档）：A Continue → 二层+继续向上入口+骷髅战士已消失+无附近威胁/B 继续向上解锁三层+入口消失+二层移动按钮含黑石塔三层 enabled/C 前往三层+只看到骷髅女妖 Lv.5 迎战/D 记录战前 Lv/maxHP/maxMP/gold/rabbit_path/E 0.99 确定性骷髅女妖战胜利返回冒险/F 女妖消失+无附近威胁+三层剧情（骷髅女妖倒在破碎的石柱之间/灰尘覆盖的项链/正是王财所说的夔峒项链/夔峒项链 ×1 已获得/当前目标：返回天龙城，将夔峒项链交还王财）/G 任务进行中+黑石塔三层：骷髅女妖已击败+黑石塔三层：已找到夔峒项链+当前目标交还王财+无可完成/提交/H 战前后 Lv/maxHP/maxMP/gold/rabbit_path 精确全不变+夔峒项链 ×1+黄金主线进行中/I Save/Continue 后当前位置 floor3+日志最终态保持+夔峒项链 ×1+无附近威胁+黄金主线进行中；另按任务卡到期窄改 P1-028-D/E/H 断言：三层【待开放】改为「继续向上」入口（无该句+入口存在）、当前目标改为击败骷髅女妖、读档日志最终态改；P1-028 其余核心回归保留）
- TM-P1-029-R1（对齐三层「继续向上」UI guard，修复审计发现的 dead button——GamePage 原按钮只检查 warrior+floor2+target flag，未含全部前序，异常存档下点击会被 Store 拒绝）：GamePage 建立窄条件 canUnlockTowerFloor3（world=floor2 + towerQuestInProgress + wangcaiBriefed + towerUnlocked + towerFloor2Unlocked + floor1_soldier/captain + floor2_zombie/mage/warrior 全部 ===true + towerFloor3UnlockPending），「继续向上」按钮只在此条件成立时显示；不复制第二套宽松逻辑；不动 gameStore.ts/App.tsx/locations/enemies/items/quests/schema/SAVE_VERSION（Store 实现正确）；E2E 补窄负路径 5 项（P1-029-R1：合法→入口存在、stage=1→不显示、briefed=false→不显示、mage=false→不显示、floor3_unlocked="yes"→不显示、恢复合法档→入口重现；注入经 page.evaluate 在浏览器侧执行 new Function）

TM-P1-030（Phase 1 收口：交还夔峒项链 → 向马科复命 → 第五主线完成 → Phase 1 明确结束 → 完整真实玩家路线回归）：
- Store 新增 returnKuidongNecklaceToWangcai()：天龙城 + 第五主线 in_progress/stage 0 + wangcai_briefed===true + 黑石塔一二三层全部解锁 + 一层两敌/二层三敌/三层骷髅女妖全部严格 true + 背包合法持有夔峒项链（唯一 entry、quantity===1）+ 目标 flag kuidong_necklace_returned undefined/false → 成功原子完成：删除夔峒项链 + kuidong_necklace_returned=true + status→completable（stage 保持 0）；目标 flag 已 true 或非 boolean（"yes"/1/0.5）整次拒绝且完全不变（不修复）；无金币/XP/等级/装备/关系值/其他奖励；player/equipment/npcStates/completedEvents/其他 world.flags/quests（除本任务 status）全不变；不自动保存
- 王财剧情：天龙城与王财交互出现「将夔峒项链交还王财」按钮（canReturnNecklaceToWangcai 窄守卫与 Store 前置一致，无 dead button）；成功后固定剧情五句（王财接过项链…黑石塔里的情况，也请你告诉马科队长）；交还后背包不再存在夔峒项链；日志新增态「夔峒项链：已交还王财。/当前目标：返回武馆，向马科复命。」
- 向马科复命：不建新提交系统——交还项链后 status=completable，直接复用现有 generic「提交任务」（quest_wangcai_trouble 原 giver 是马科）；提交后 status=completed；无临时 Phase 1 奖励系统（该任务无 goldReward）；完成后显示马科短剧情（马科听完黑石塔里的经过…黑石塔的调查暂时告一段落）+ 明确显示「第一阶段完成」「当前可玩主线内容已完成。/《追寻黄金兔子王》将在后续阶段继续。」；黄金兔子任务不 completed/failed、不改其 flag/stage/兔子的路径（保持长期线冻结）
- GamePage 王财任务日志块条件由 in_progress 扩展为 in_progress/completable（交还后仍显示「已交还/回武馆复命」），completed 由完成剧情块接管
- 本卡禁止扩张：未建 黑石塔四层/新地图/新怪/新 NPC/新任务/黄金兔子目的地/莉安雅等/队友/关系/宠物/职业系统/Loot/Dungeon/BossEngine/任务框架重构/schema/SAVE_VERSION/新依赖/UI 重做
- schema 不变、SAVE_VERSION=1；quests.ts/npcs.ts/professions.ts/CombatPage.tsx/types/storage/rules/locations/enemies/items 零修改；git diff 仅 gameStore.ts + gameStore.test.ts + GamePage.tsx + qa/e2e.mjs + qa/phase1-playthrough.mjs(新增) + README（不含 qa/*.png）
- Store 单测新增 X 组（returnKuidongNecklaceToWangcai：X1 合法原子完成（删项链+flag=true+status=completable+stage 保持 0）/X2 it.each 21 项拒绝 false 且完整不变（错误 location/任务不存在/available/stage 1/briefed false/三层某解锁 false/一层两敌/二层三敌/witch false/背包无项链/项链 quantity 2/项链两条 entry/returned true/"yes"/1/0.5）/X3 无金币/XP/等级/装备奖励/X4 重复交还无副作用项链不再出现/X5 黄金深比较不变/X6 不 autosave）
